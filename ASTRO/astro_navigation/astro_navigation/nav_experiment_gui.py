...
class NavExperimentGui(Node):
    def __init__(self):
        ...
        self.namespace = 'sim'
        self.initialpose_topic = f'/{self.namespace}/initialpose'
        self.reset_positions_service = f'/{self.namespace}/reset_positions'
        ...
        self.reset_positions_client = self.create_client(
            Empty,
    ...
    def _launch_command(self, planner, controller):
        return [
            'ros2', 'launch', 'astro_navigation', 'navigation_stage.launch.py',
    ...
    def _benchmark_command(self, run_name, planner, controller):
        return [
            'ros2', 'run', 'astro_navigation', 'nav_benchmark_runner.py', '--ros-args',
    ...
    def _call_reset_positions_service(self):
        if not self.reset_positions_client.wait_for_service(timeout_sec=2.0):
            raise RuntimeError(
                f'Servis {self.reset_positions_service} nije dostupan.'
            )

        req = Empty.Request()
        future = self.reset_positions_client.call_async(req)
        ...
        while rclpy.ok() and not future.done():
            ...
            if time.time() - t0 > 5.0:
                raise RuntimeError(
                    f'Timeout pri pozivu servisa {self.reset_positions_service}.'
                )

        if future.result() is None:
            raise RuntimeError(
                f'Poziv servisa {self.reset_positions_service} nije uspio.'
            )
    ...
    def on_enter(self):
        ...
        try:
            ...
            self.reset_done = False
            self.active_var.set(
                f'Aktivno: planner={planner}, controller={controller} (ns={self.namespace})'
            )
            self.set_status(
                f'Pokrenut navigation launch za {planner} + {controller} u namespace-u {self.namespace}. '
    ...
    def on_reset(self):
        try:
            self._call_reset_positions_service()
            time.sleep(0.2)
        except Exception as exc:
            messagebox.showerror('Greška pri resetu Stage-a', str(exc))
            self.set_status(f'Greška pri pozivu {self.reset_positions_service}: {exc}')
            self._refresh_start_button()
            return
        ...
        self.set_status(
            f'Napravljen Stage reset preko {self.reset_positions_service} i objavljen {self.initialpose_topic}.'
        )
        self._refresh_start_button()
    ...
    def on_start(self):
        ...
        try:
            ...
            self.set_status(
                f'Pokrenut benchmark: {run_name} u namespace-u {self.namespace}'
            )
        except Exception as exc:
            self.benchmark_proc = None
